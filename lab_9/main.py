...
def plot_local_energy(aud, window_function, sample_rate):
    sqvr = aud ** 2
...
if __name__ == "__main__":
    wav_file_path = 'а_муж.wav'
    aud, fs = librosa.load(wav_file_path)
    ...
    plot_spectrogram(aud, magnitude, window, fs, hop_size)

    print(get_formants(aud, fs))

    spectrum = np.fft.rfft(aud)

    plt.plot(np.fft.rfftfreq(len(aud), 1. / fs), np.abs(spectrum) / len(aud))
    plt.xlim([0, 4000])
    ...
    # spectrum = np.fft.fft(aud)
    # cepstrum = np.fft.ifft(np.log(np.abs(spectrum))).real
    # ms1 = fs / 1000
    # ms20 = fs / 50
    # plt.plot(np.abs(cepstrum)[:window//2])
    # plt.show()
